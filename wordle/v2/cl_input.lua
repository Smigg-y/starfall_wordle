...
local function findHovered(group, cx, cy)
	for i = 1, #group do
		local c = group[i]
		if c:containsPoint(cx, cy) then return c end
	end
end
...
function InputManager:update(activeGroup)
	...
	if activeGroup then
		...
		if group then hovered = findHovered(group, cx, cy) end
	else
	...
	if hovered ~= self.hovered then
		if self.hovered then self.hovered:onHover(false) end
		if hovered then hovered:onHover(true) end
