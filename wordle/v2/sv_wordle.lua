...
--@includedir wordle/data

if not SERVER then return end

local WordleUtil                              = require("wordle/v2/sh_wordle.lua")

local WordleGuesses                           = require("wordle/data/valid.txt")
local WordleAnswers                           = require("wordle/data/choices.txt")

local Config, States, NetNames, ErrorCodes    = WordleUtil.Config, WordleUtil.States, WordleUtil.NetNames,
		WordleUtil.ErrorCodes
local wordLength, maxGuesses, autoScreenSetup = Config.wordLength, Config.maxGuesses, Config.autoScreenSetup
...
function Wordle:start(ply)
	...
	print(self.answer)
	self.answerBytes = { string.byte(self.answer, 1, wordLength) }
	self.player      = ply
...
function Wordle:evaluateGuess(guess)
	local gbytes = { string.byte(guess, 1, wordLength) }
	local abytes = self.answerBytes

	local remaining = {}

	for i = 1, wordLength do
		local g, a = gbytes[i], abytes[i]
		if g ~= a then
...
net.receive(NetNames.StartGame, function(_, ply)
	if isValid(ply) then
		wordle:start(ply)
	end
end)
...
net.receive(NetNames.KeyTyped, function(_, ply)
	...
	net.writeUInt(net.readUInt(5), 5)
	net.send()
end)
...
if autoScreenSetup then
	...
	local function createScreen(origin, angle, cb)
		local scr = prop.createComponent(origin, angle, "starfall_screen", "models/hunter/plates/plate2x2.mdl", true)
		cb(scr)
	end
	...
	local function setupScreen(scr, r_mode)
		...
		if r_mode then
			scr:setRenderMode(r_mode)
		end
	end

	local ch_up = ch:getUp()
	local screen_origin = ch:localToWorld(Vector(0, 0, 64))
	local screen_angle = (owner():getEyePos() - screen_origin):getNormalized():getAngleEx(ch_up)

	screen_angle.p = 75

	if ch_up:dot(Vector(0, 0, 1)) <= 0.7 then
		screen_angle.r = 0
	end
	...
	createScreen(screen_origin, screen_angle, function(scr)
		setupScreen(scr)
		createScreen(scr:localToWorld(Vector(36, 0, 4)), scr:localToWorldAngles(Angle(-45, 0, 0)), function(kb)
